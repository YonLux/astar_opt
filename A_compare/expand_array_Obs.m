function exp_array=expand_array_Obs(node_x,node_y,hn,xTarget,yTarget,CLOSED,MAX_X,MAX_Y,Nobs,xStart,yStart,P_obs, P_tong)
                ...
                if( (s_x >0 && s_x <=MAX_X) && (s_y >0 && s_y <=MAX_Y))    %node within array bound   %%% 数组绑定中的节点，在坐标内
                   ...
                   if (flag == 1)   
                       ...
                       % t=angle6(p_xNode,p_yNode,node_x,node_y,s_x, s_y);
                        ...
                        ou(exp_count,5) = P_tong*ou(exp_count,3) + (1+P_obs)*ou(exp_count,4)+ (r1/R)*ou(exp_count,4);
                   end
